    ...
    public sealed class UISystem : ModSystem
    {
        ...
        private bool DrawStaticUI()
        {
            if (Main.gameMenu || !ModContent.GetInstance<Configs.Config>().EnableUI)
                return true;
            if (SoulBarsUI.SoulBars[0].TopBar != ModContent.GetInstance<Configs.Config>().UIYPos + 38f || SoulBarsUI.SoulBars[0].LeftBar != ModContent.GetInstance<Configs.Config>().UIXPos + 6f)
